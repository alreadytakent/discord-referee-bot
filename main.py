...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Game(name=".dth, .mdth, .contra @opponent to play!"))
...
async def show_score(ctx, *, args=None):
    ...
    if ' vs ' in player_mention.lower():
        ...
        for game_type, total, wins1, wins2, draws in stats:
            ...
            if game_type in ['dth', 'mdth', 'dotty', 'comb', 'contr']:  # Added contradiction here
                draws_display = "-"
            else:
...
@bot.command(name='contr')  # Add this new command
...
@start_contradiction_game.error  # Add error handler for Contradiction
async def start_game_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send("❌ Please mention an opponent! Usage: `.dth @opponent`, `.mdth @opponent`, `.contra @opponent`, or `.gops @opponent`")
    elif isinstance(error, commands.BadArgument):
        await ctx.send("❌ Please mention a valid user! Usage: `.dth @opponent`, `.mdth @opponent`, `.contra @opponent`, or `.gops @opponent`")
    else:
