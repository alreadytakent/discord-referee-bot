import discord
import random
from game_stats import record_game_result  # Add this import


class MangaAccurateDTHGame:
    def __init__(self, player1, player2, channel):
        ...
        self.round = 1
        self.main_vessel = {player1.id: 0, player2.id: 0}
        self.accumulated_vessel = {player1.id: 0, player2.id: 0}
    ...
    async def start_game(self):
        ...
        await self.channel.send(
            f"🎮 **Manga Accurate Drop the Handkerchief** game started between {self.player1.mention} and {self.player2.mention}!\n"
    ...
    async def _calculate_results(self, check_time):
        ...
        # Calculate penalty
        if check_time >= self.dropped_number:
            # Successful check
            penalty = check_time - self.dropped_number
            result_type = "SUCCESSFUL CHECK"

            # Add penalty to accumulated vessel
            ...
            transfer_amount = self.accumulated_vessel[self.checker.id]
            self.main_vessel[self.checker.id] += transfer_amount
        ...
        result_message = (
            ...
            f"~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~\n"
            f"{self.player1.mention} ({self.main_vessel[self.player1.id]}/{self.max_penalty})\n"
            f"Vessel: {self.accumulated_vessel[self.player1.id]}/{self.max_penalty}\n"
            f"~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~\n"
            f"{self.player2.mention} ({self.main_vessel[self.player2.id]}/{self.max_penalty})\n"
            f"Vessel: {self.accumulated_vessel[self.player2.id]}/{self.max_penalty}"
        )

        # Check for game end (either vessel reaches max penalty)
        if (self.main_vessel[self.checker.id] >= self.max_penalty or
                self.accumulated_vessel[self.checker.id] >= self.max_penalty):
            winner = self.dropper  # The dropper wins when checker reaches max penalty in either vessel
            result_message += f"\n\n🏆 **GAME OVER!** {winner.mention} wins!\n"
            if self.main_vessel[self.checker.id] >= self.max_penalty:
                result_message += f"{self.checker.mention} reached 300 seconds penalty!"
            else:
                result_message += f"{self.checker.mention}'s vessel reached 300 seconds penalty!"
            ...
            if winner.id == self.player1.id:
                ...
                result_code = 2
            record_game_result('mdth', self.player1.id, self.player2.id, result_code)
